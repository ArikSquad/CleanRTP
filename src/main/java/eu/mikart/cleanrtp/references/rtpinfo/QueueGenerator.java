...
public class QueueGenerator {
    ...
    private void queueGenerator(ReQueueData data) {
        ...
        task = AsyncHandler.asyncLater(() -> {
            ...
            for (RtpSetupType setup : RtpSetupType.values()) {
    ...
    private static HashMap<String, RTPWorld> getFromSetup(RtpSetupType type) {
        switch (type) {
    ...
    private static String getId(RtpSetupType type, String id) {
        switch (type) {
                ...
                loc.getWorld().getChunkAtAsync(loc)
                        .thenAccept(v -> {
                            Location safeLoc = RandomLocation.getSafeLocation(
                                    RtpHelper.getWorldType(rtpWorld.getWorld()),
                                    loc.getWorld(),
                                    loc,
                                    rtpWorld.getMinY(),
                                    rtpWorld.getMaxY(),
